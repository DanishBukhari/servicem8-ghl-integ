...
const checkNewContacts = async () => {
  try {
    ...
    for (const contact of contacts) {
      ...
      if (!contactName) {
        console.log(`No name for contact ${contactUuid}, skipping GHL creation.`);
  ...
  try {
    ...
    for (const job of completedJobs) {
      ...
      try {
        ...
        primaryContact = contacts.find(c => c.email) || contacts.find(c => c.phone || c.mobile) || contacts[0] || {};
      ...
      if (!ghlContactId) {
        try {
          ...
          if (searchQuery) {
            ...
            console.log(`No search query available for job ${jobUuid}, skipping webhook.`);
            continue;
          }
        } catch (error) {
