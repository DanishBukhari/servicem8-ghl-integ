...
const REDIRECT_URI =
  process.env.GHL_REDIRECT_URI || `http://localhost:${PORT}/callback`;
...
async function updateHerokuConfig(tokens) {
  if (!APP_NAME || !HEROKU_API_KEY) {
    console.log(
      "Heroku APP_NAME or HEROKU_API_KEY not set; skipping Heroku config update."
    );
    return;
  }
  ...
  try {
    ...
    const payload = {
      ...
      GHL_TOKEN_CREATED_AT: tokens.created_at
        ? tokens.created_at.toString()
        : `${Math.floor(Date.now() / 1000)}`,
      GHL_TOKEN_EXPIRES_IN: tokens.expires_in
    ...
    console.error(
      "Error updating Heroku config vars:",
...
function loadTokens() {
  ...
  if (process.env.GHL_ACCESS_TOKEN && process.env.GHL_REFRESH_TOKEN) {
    ...
    return {
      ...
      created_at:
        parseInt(process.env.GHL_TOKEN_CREATED_AT, 10) ||
        Math.floor(Date.now() / 1000),
  ...
  if (now >= expiry) {
    ...
    try {
      ...
      if (errData && errData.error === "invalid_grant") {
        console.error(
          "Refresh token invalid. You must re-authorize via /auth to get new tokens."
        );
        ...
        try {
          if (fs.existsSync(TOKEN_FILE)) {
            ...
            console.log(
              "Removed local tokens.json due to invalid refresh token."
            );
          }
        } catch (unlinkErr) {
...
async function fetchAll(endpoint, queryParams = {}) {
  ...
  while (cursor) {
    ...
    const response = await serviceM8Api.get(
...
async function loadState() {
  try {
    ...
    console.log(
      "Loaded state: processedJobs=",
      processedJobs.size,
      "processedContacts=",
      processedContacts.size
    );
...
const checkNewContacts = async () => {
  try {
    ...
    for (const contact of contacts) {
      ...
      if (!contactName) {
        console.log(
          `No name for contact ${contactUuid}, skipping GHL creation.`
        );
      ...
      try {
        if (email) {
          ...
          if (existingContact) {
            ...
            console.log(
              `Contact already exists in GHL: ${ghlContactId} for email ${email}`
            );
            processedContacts.add(contactUuid);
            continue;
          }
      ...
      let addressDetails = {};
      try {
        const company = await fetchAll("company.json", {
        ...
        console.log(
          `Fetched company address for ${company_uuid}:`,
          addressDetails
        );
      } catch (error) {
        console.error(
...
const checkJobCompletions = async () => {
  try {
    ...
    const completedJobs = await fetchAll("job.json", {
      $filter: "status eq 'Completed'",
    });
    console.log(
      `Fetched ${completedJobs.length} completed jobs from ServiceM8`
    );
    ...
    for (const job of completedJobs) {
      ...
      console.log(
        `Fetched job ${jobUuid}: status=${job?.status}, company_uuid=${job?.company_uuid}, edit_date=${job?.edit_date}`
      );

      let completionDate = null;
      try {
        const jobActivities = await fetchAll("jobactivity.json", {
          $filter: `job_uuid eq '${jobUuid}'`,
        });
        console.log(
          `Fetched ${jobActivities.length} activities for job ${jobUuid}`
        );
        let maxEndDate = null;
        for (const activity of jobActivities) {
          if (activity.end_date) {
            console.log(
              `Activity for job ${jobUuid}: end_date=${activity.end_date}`
            );
        ...
        if (maxEndDate) {
          ...
          console.log(
            `Using job activity end_date as completion date: ${completionDate.format(
              "YYYY-MM-DD HH:mm:ss"
            )}`
          );
        } else {
          ...
          console.log(
            `No activities found, using job edit_date as completion date: ${completionDate.format(
              "YYYY-MM-DD HH:mm:ss"
            )}`
          );
        }
        ...
        console.error(
          `Error fetching job activities for job ${jobUuid}:`,
          error.response ? error.response.data : error.message
        );
        completionDate = moment(job.edit_date).tz(accountTimezone);
        console.log(
          `Failed to fetch activities, using job edit_date as completion date: ${completionDate.format(
            "YYYY-MM-DD HH:mm:ss"
          )}`
        );
      }
      ...
      if (!completionDate) {
        console.log(
          `No completion date available for job ${jobUuid}, skipping.`
        );
        continue;
      }
      console.log(
        `Job ${jobUuid} completion date: ${completionDate.format(
          "YYYY-MM-DD HH:mm:ss"
        )}, target: ${targetDate.format("YYYY-MM-DD HH:mm:ss")}`
      );
      if (!completionDate.isSameOrAfter(targetDate)) {
        console.log(
          `Job ${jobUuid} not completed on or after May 24, 2025, skipping.`
        );
        continue;
      }
      ...
      if (job.category_uuid) {
        try {
          const categoryResponse = await serviceM8Api.get(
            `/category.json?$filter=uuid eq '${job.category_uuid}'`
          );
          ...
          console.error(
            `Error fetching category for job ${jobUuid}:`,
            error.response ? error.response.data : error.message
          );
        }
      }
      ...
      if (
        categoryName.includes("real estate agent") ||
        categoryName.includes("property manager")
      ) {
        console.log(
          `Skipping webhook for job ${jobUuid} as category includes Real Estate Agent or Property Manager`
        );
      ...
      if (job.job_description) {
        const ghlContactIdMatch = job.job_description.match(
          /GHL Contact ID: ([a-zA-Z0-9]+)/
        );
        ghlContactId = ghlContactIdMatch ? ghlContactIdMatch[1] : "";
        console.log(
          `Extracted GHL Contact ID: ${ghlContactId} for job ${jobUuid}`
        );
      }
      ...
      try {
        ...
        console.log(
          `Extracted client email: ${clientEmail} for company ${companyUuid}`
        );
      } catch (error) {
        console.error(
          `Error fetching contacts for company ${companyUuid}:`,
          error.response ? error.response.data : error.message
        );
      }
      ...
      if (!ghlContactId && clientEmail) {
        try {
          ...
          if (existingContact) {
            ghlContactId = existingContact.id;
            console.log(
              `Found existing GHL contact: ${ghlContactId} for email ${clientEmail}`
            );
          } else {
            ...
            try {
              ...
              console.log(
                `Fetched company address for ${companyUuid}:`,
                addressDetails
              );
            } catch (error) {
              console.error(
                `Error fetching company details for ${companyUuid}:`,
                error.response ? error.response.data : error.message
              );
            }

            // Create new contact in GHL
            const contactName = `${primaryContact.first || ""} ${
              primaryContact.last || ""
            }`.trim();
            const ghlContactResponse = await ghlApi.post("/contacts/", {
            ...
            ghlContactId = ghlContactResponse.data.contact.id;
            console.log(
              `Created GHL contact: ${ghlContactId} for email ${clientEmail}`
            );
            ...
            try {
              ...
              console.log(
                `Updated job ${jobUuid} description with GHL Contact ID: ${ghlContactId}`
              );
            } catch (updateError) {
              console.error(
                `Failed to update job ${jobUuid} description:`,
          ...
          console.error(
            `Error handling GHL contact for email ${clientEmail}:`,
            error.response ? error.response.data : error.message
          );
        ...
        console.log(
          `No GHL Contact ID or email for job ${jobUuid}, skipping webhook.`
        );
        continue;
      }
      ...
      if (contactKey && processedContacts.has(contactKey)) {
        console.log(
          `Contact ${contactKey} already triggered, skipping job ${jobUuid}`
        );
        continue;
      }
      ...
      if (completionDate.isAfter(twentyFourHoursAgo)) {
        console.log(
          `Recent completed job found: UUID ${jobUuid}, Completion Date ${completionDate.format(
            "YYYY-MM-DD HH:mm:ss"
          )}`
        );
        ...
        try {
          ...
          console.log(
            `GHL webhook triggered for job ${jobUuid}: status=${
              webhookResponse.status
            }, response=${JSON.stringify(webhookResponse.data)}`
          );
          processedJobs.add(jobUuid);
        ...
        console.log(
          `Job ${jobUuid} is not recently completed, skipping. Details:`,
          {
    ...
    console.error(
      "Error checking job completions:",
      error.response ? error.response.data : error.message
    );
  }
};
...
app.post("/ghl-create-job", upload.array("photos"), async (req, res) => {
  try {
    ...
    if (lastProcessed && now - lastProcessed < 5000) {
      console.log(
        `Duplicate job creation attempt for ghlContactId ${ghlContactId}, skipping`
      );
      return res
        .status(200)
        .json({ message: "Job creation skipped (duplicate request)" });
    }
    ...
    if (!queueUuid) {
      ...
      return res
        .status(500)
        .json({
          error: 'Failed to fetch "To Be Quoted (Desk Quote)" queue UUID',
        });
    }
    ...
    if (matchingCompany) {
      ...
      console.log(
        `Creating new client with name ${fullName}, email ${email}, phone ${phone}`
      );
      ...
      console.log(
        `Client created: ${companyUuid} for email ${email} with phone ${phone}`
      );
    }

    const existingContacts = await fetchAll("companycontact.json", {
      $filter: `company_uuid eq '${companyUuid}'`,
    });
    ...
    if (!matchingContact) {
      ...
      console.log(
        `Contact already exists for company ${companyUuid}, skipping creation.`
      );
    }
    ...
    try {
      ...
      console.log(
        `Fetched GHL contact data for ${ghlContactId}:`,
      ...
      if (!Array.isArray(customFields)) {
        console.log(
          `Custom fields not an array, attempting to convert object:`,
          customFields
        );
        customFields = Object.values(customFields).filter(
          (f) => f && typeof f === "object"
        );
      }

      if (customFields.length > 0) {
        const messageField = customFields.find(
          (field) =>
            (field.name &&
              (field.name === "Message" ||
            ...
            (field.label &&
              (field.label === "Message" ||
                field.label.toLowerCase() === "message")) ||
            (field.id &&
              (field.id === "zNzhT7M36keauEw2TCtf" ||
                field.id === "VvhUQGlzD80PnB9aYdL4"))
        );
        if (messageField && (messageField.value || messageField.values)) {
          message =
            messageField.value ||
            (messageField.values && messageField.values[0]) ||
            "";
          console.log(
            `Message retrieved for contact ${ghlContactId}: ${message}`
          );
        } else {
          console.log(
            `No Message field found in customFields for contact ${ghlContactId}:`,
            customFields
          );
        }

        const urgencyField = customFields.find(
          (field) => field.id === "wqA4BoeymIy9Vhi6E5t3"
        );
        if (urgencyField && (urgencyField.value || urgencyField.values)) {
          urgency =
            urgencyField.value ||
            (urgencyField.values && urgencyField.values.join(", ")) ||
            "";
          console.log(
            `Urgency retrieved for contact ${ghlContactId}: ${urgency}`
          );
        } else {
          console.log(
            `No Urgency field found in customFields for contact ${ghlContactId}`
          );
        }

        const sourceField = customFields.find(
          (field) => field.id === "1EkwGnC9UtMNrTCwGnBk"
        );
        if (sourceField && (sourceField.value || sourceField.values)) {
          source =
            sourceField.value ||
            (sourceField.values && sourceField.values.join(", ")) ||
            "";
          console.log(
            `Source retrieved for contact ${ghlContactId}: ${source}`
          );
        } else {
          console.log(
            `No Source field found in customFields for contact ${ghlContactId}`
          );
        }
      } else {
        console.log(
          `No customFields available or accessible for contact ${ghlContactId}`
        );
      }
    } catch (error) {
      console.error(
        "Failed to fetch contact message from GHL:",
    ...
    try {
      ...
      if (contact.customField) {
          ...
          if (
            ...
            for (const [uuid, entry] of Object.entries(field.value)) {
              if (
                ...
                const fileExtension = getFileExtensionFromMime(
                  entry.meta.mimetype
                );
                photoData.push({
                  url: entry.url,
                  ...
                  filename:
                    entry.meta.originalname ||
                    `photo-${uuid}-${Date.now()}${fileExtension}`,
        ...
        console.log(
          `No customField found in GHL contact ${ghlContactId}. Available properties: ${Object.keys(
            contact
          )}`
        );
      }
      ...
      if (photoData.length === 0) {
        try {
          const attachmentsResponse = await ghlApi.get(
            `/contacts/${ghlContactId}/attachments`
          );
          const attachments = attachmentsResponse.data.attachments || [];
          console.log(
            `Fetched ${attachments.length} attachments from GHL contact ${ghlContactId}`
          );
          ...
          for (const attachment of attachments) {
            if (
              ...
              photoData.push({
                url: attachment.url,
                documentId:
                  attachment.documentId || attachment.url.split("/").pop(),
                filename:
                  attachment.filename ||
                  `attachment-${Date.now()}${fileExtension}`,
          ...
          console.log(
            "Attachments endpoint not available or failed:",
      ...
      console.log(
        `Fetched ${photoData.length} photos for contact ${ghlContactId}:`,
    ...
    for (const photo of photoData) {
      ...
      try {
        ...
        try {
          downloadResponse = await axios.get(
            `https://services.leadconnectorhq.com/documents/download/${documentId}`,
            {
              headers: {
                Authorization: `Bearer ${await getAccessToken()}`,
              },
          ...
          console.log(
            `Primary download failed for ${photoUrl}:`,
          ...
          try {
            ...
            console.error(
              `Fallback download failed for ${photoUrl}:`,
        ...
        if (!contentType.match(/image\/(png|jpeg|jpg)/i)) {
          console.log(
            `Skipping non-image URL ${photoUrl}: Content-Type ${contentType}`
          );
          continue;
        }
        ...
        console.log(
          `Downloaded image to ${tempPath}, size: ${stats.size} bytes`
        );
        if (stats.size === 0) {
        ...
        const attachmentResponse = await serviceM8Api.post(
          "/Attachment.json",
          attachmentData
        );
        attachmentUuid = attachmentResponse.headers["x-record-uuid"];
        console.log(
          `Created attachment record for job ${jobUuid}, UUID: ${attachmentUuid}, file_type: ${fileExtension}`
        );
...
app.post("/ghl-appointment-sync", async (req, res) => {
  ...
  try {
    console.log(
      "Appointment startTime string from GHL:",
      appointment.startTime
    );
    ...
    const endTime = moment(appointment.endTime, "dddd, MMMM D, YYYY h:mm A").tz(
      "Australia/Brisbane"
    );
    console.log(
      "Parsed startTime:",
    ...
    let contact;
    try {
      const contactResponse = await ghlApiV2.get(
        `/contacts/${appointment.contactId}`
      );
      contact = contactResponse.data.contact;
      console.log(
        `Fetched GHL contact: ${contact.id}, email: ${contact.email}, locationId: ${contact.locationId}`
      );
    } catch (error) {
      console.error(
        `Error fetching GHL contact ${appointment.contactId}:`,
        error.response?.data || error.message
      );
    ...
    const lastName = contact.lastName
      ? contact.lastName.charAt(0).toUpperCase() +
        contact.lastName.slice(1).toLowerCase()
      : "";
    ...
    try {
      ...
      console.error(
        "Error syncing ServiceM8 contact:",
        error.response?.data || error.message
      );
    ...
    const jobData = {
      company_uuid: companyUuid,
      status: "Work Order",
      queue_uuid: "80e5981d-e2fb-470b-9577-23231dab715b", // a67d1770-7a34-461a-91d9-23393bfa8d8b
      job_address:
        appointment.location || contact.address1 || "No address provided",
    ...
    try {
      ...
      console.log(
        `Created ServiceM8 job: ${jobUuid}, status: ${jobData.status}`
      );
    } catch (error) {
      console.error(
        "Error creating ServiceM8 job:",
        error.response?.data || error.message
      );
    ...
    try {
      ...
      console.error(
        "Error adding job contact to ServiceM8:",
        error.response?.data || error.message
      );
    ...
    const sebastianUuid = "431dbebf-23ff-477a-9559-1fab55a816bb"; // bdb4d3b3-3190-41ca-9e1b-2339396aa3eb
    const tamsinUuid = "979f57c6-cfe4-4ef5-b002-208f2994fa8b"; // 3899bd2d-65dc-416d-aa4d-21e1a7706aeb

    // Function to check staff availability
    const isStaffAvailable = async (staffUuid) => {
      try {
        const activities = await fetchAll("jobactivity.json", {
          $filter: `staff_uuid eq '${staffUuid}'`,
        });

        console.log(
          `Fetched ${activities.length} activities for staff ${staffUuid}`
        );

        const appointmentDate = startTime.clone().startOf("day");
        for (const activity of activities) {
          const activityStart = moment(activity.start_date).tz(
            "Australia/Brisbane"
          );
          const activityEnd = moment(activity.end_date).tz(
            "Australia/Brisbane"
          );
          if (activityStart.isSame(appointmentDate, "day")) {
            if (
              ...
              (startTime.isSameOrBefore(activityStart) &&
                endTime.isSameOrAfter(activityEnd))
            ) {
              console.log(
                `Overlap found for staff ${staffUuid}: ${activity.start_date} - ${activity.end_date}`
              );
        ...
        console.error(
          `Error checking availability for staff ${staffUuid}:`,
          error.response?.data || error.message
        );
    ...
    if (await isStaffAvailable(sebastianUuid)) {
      ...
      console.log(
        "Sebastian is not available, but Tamsin is, booking with her."
      );
    } else {
      console.log("Neither Sebastian nor Tamsin is available.");
      return res
        .status(200)
        .json({ message: "No available slot in ServiceM8" });
    }
    ...
    console.log(
      "Sending to ServiceM8: start_date=",
    ...
    try {
      const response = await serviceM8Api.post(
        "/jobactivity.json",
        activityData
      );
      const activityUuid = response.headers["x-record-uuid"];
      console.log(
        `Created ServiceM8 job activity: ${activityUuid} for appointment ${appointmentId}, staff: ${selectedStaffUuid}`
      );
      console.log(
        `Activity details: start_date=${activityData.start_date}, end_date=${activityData.end_date}, job_uuid=${jobUuid}, scheduled=${activityData.activity_was_scheduled}, active=${activityData.active}`
      );

      // Fetch the created activity to check stored dates
      const fetchedActivityResponse = await serviceM8Api.get(
        `/jobactivity/${activityUuid}.json`
      );
      const fetchedActivity = fetchedActivityResponse.data;
      console.log(
        "Stored in ServiceM8: start_date=",
        fetchedActivity.start_date,
        "end_date=",
        fetchedActivity.end_date
      );
      ...
      res
        .status(200)
        .json({ message: "Appointment synced", jobUuid, activityUuid });
    } catch (error) {
      console.error(
        "Error creating ServiceM8 job activity:",
        error.response?.data || error.message
      );
